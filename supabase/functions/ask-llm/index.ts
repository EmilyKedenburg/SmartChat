...
import { GoogleGenerativeAI } from "npm:@google/generative-ai"; // Corrected import
...
serve(async (req) => {
  ...
  try {
    ...
    const genAI = new GoogleGenerativeAI(Deno.env.get("GOOGLE_API_KEY")!); // Changed to GOOGLE_API_KEY
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro" }); // Changed model to gemini-1.5-pro
